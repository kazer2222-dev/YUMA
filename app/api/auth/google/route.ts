...
export async function GET(request: NextRequest) {
  ...
  // Get rememberMe and mode from query params (set by client before redirect)
  const rememberMe = request.nextUrl.searchParams.get('rememberMe') === 'true';
  const authMode = request.nextUrl.searchParams.get('mode') || 'signin'; // 'signin' or 'signup'
  ...
  // Store auth mode (signin or signup)
  response.cookies.set('oauth_mode', authMode, {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
    path: '/',
